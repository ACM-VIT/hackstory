...
const TrackContent = () => {
	...
	const tracks: Track[] = [
		{
			name: "FINTECH",
			description:
				"Leverage technology to modify, enhance or automate processes in finance. This is your chance to make transactions more accessible and affordable utilising technologies of your choice. Streamline processes, come up with smoother financial systems and win big.",
		...
		},
		{
			name: "GAME OF LIFE",
			description:
				"A concept that focuses on developing game-based products that put the fun back in mundane and difficult processes. Choose 1 from 3 different problem statements. Let your inner child guide you to make the product experience as enjoyable as possible.",
		...
		},
		{
			...
			description:
				"Use ML as a powerful innovation tool to combat modern-day social issues that affect any section of society. With the diverse capablities of ML, harness it to your full extent and contribute towards the upliftment of society through technology.",
			oneAns: "Design an ML-based Income Range Predictor with Bias-Aware Integration: <br/>Develop a machine learning solution to predict job applicants' income range based on their demographic and professional information. The solution should include a user-friendly interface, either as a website or SDK. <br/> Address potential biases to ensure fair and unbiased predictions. Conduct Exploratory Data Analysis (EDA) to understand the dataset, identify biases, and analyze variable relationships. Mitigate bias during model training and evaluation.<br />Dataset link: https://www.kaggle.com/datasets/architlahiri/hackstory-problem-1",
	...
	const [trackOpenState, setTrackOpenState] = useState<boolean[]>(
			...
			<div className="w-[80vw]">
				...
				<div className="">
					{tracks.map((track, index) => (
						...
						>
							...
							>
								<div className="flex  flex-col  gap-y-4 pr-12 text-left font-bold sm:pr-36">
									<div>
										<p
											className={`text-3xl text-yellow lg:text-2xl ${oswald.className}`}
										>
											{track.name}
										</p>
										<p
											className={`text-lg text-[#A7A9BE] ${manrope.className}`}
										>
											{track.description}
										</p>
									</div>
				...
				<div className="text-left">
					<div className="rounded border-b border-t border-gray-300 border-opacity-10 px-4 py-6">
						...
						>
							<span
								className={`pr-4 text-left text-3xl font-semibold text-yellow lg:text-2xl ${oswald.className}`}
							>
								OPEN INNOVATION
							</span>
						</button>
						<p
							className={`text-[18px] font-extrabold text-[#A7A9BE] md:mr-36 ${manrope.className}`}
						>
							Let your creativity take over with Open Innovation.
							With no constraint on industry or technology, create
							the product of your choice in the domain of your
							choice. Feel free to solve any real-world problem
