...
const TrackButtons = ({ track, buttonA, buttonB, buttonC, buttonD }: Props) => {
    ...
    return (
        <div className="flex flex-col gap-x-5 gap-y-2 max-md:items-center md:flex-row">
            ...
            <p onClick={() => handleClick(track)} className="p-2 text-trackstext">{buttonB}</p>
            <p onClick={() => handleClick(track)} className="p-2 text-trackstext">{buttonC}</p>
            <p onClick={() => handleClick(track)} className="p-2 text-trackstext">{buttonD}</p>
        </div>
    );
};
