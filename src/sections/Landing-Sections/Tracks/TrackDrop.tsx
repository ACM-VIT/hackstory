...
export const TrackDrop = ({ title, answer }: Props) => {
		...
		<div>
			<div className="text-left">
				<div className="rounded border-b border-t border-gray-300 border-opacity-10 px-4 py-6">
					<button
						className={`flex w-full items-center justify-between focus:outline-none ${manrope.className}`}
					>
						<span className="pr-4 text-left text-lg font-semibold text-white sm:text-xl">
							{title}
						</span>
					</button>
					<p
						className={`pt-4 text-[#FFF6E8] md:mr-20 ${manrope.className}`}
	...
	// 				>
	// 					<span
	// 						className={`pr-4 text-left text-lg font-semibold text-white sm:text-xl`}
	// 					>
	// 						{title}
	// 					</span>
	// 					{getIcon()}
